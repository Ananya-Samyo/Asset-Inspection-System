<template>
  ...
  <h1 class="main-title">กำหนดระยะเวลาการตรวจสอบครุภัณฑ์</h1>
  ...
  <div class="container">
    ...
    <!-- ปุ่มแก้ไข -->
    <button v-if="isSaved" class="edit-btn" @click="onEditDateClick">
      แก้ไขวันที่
    </button>
...
export default {
  ...
  methods: {
    ...
    async fetchOpeningTimes() {
      try {
        // ✅ ใช้ endpoint ใหม่
        const { data } = await axios.get(endpoints.admin.openingTimes);
    ...
    async onEditDateClick() {
      try {
        // ✅ endpoint เดิม แก้เป็น admin
        const res = await axios.get(endpoints.admin.openingTimes);
        const current = res.data;

        if (!current || !current.open_datetime || !current.close_datetime) {
          await Swal.fire({
            icon: 'info',
            title: 'ยังไม่มีการตั้งช่วงเวลา',
            text: 'โปรดบันทึกช่วงเวลาใหม่ก่อน'
          });
          return;
        }

        this.startDate = this.formatDateInput(current.open_datetime);
        this.endDate = this.formatDateInput(current.close_datetime);
        this.isAlreadySaved = false;
        this.isSaved = false;

        await Swal.fire({
          icon: 'info',
          title: 'สามารถขยายเวลาตรวจสอบได้',
          text: 'สามารถขยายวันที่สิ้นสุดของช่วงเวลาตรวจสอบได้',
        ...
        await Swal.fire({
          ...
          text: 'ไม่สามารถโหลดข้อมูลวันที่เดิมได้'
        });
      }
    },
  ...
  try {
    ...
    await axios.post(endpoints.admin.openingTimes, {
